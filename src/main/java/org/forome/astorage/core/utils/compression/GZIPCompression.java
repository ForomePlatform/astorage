...
package org.forome.astorage.core.utils.compression;
...
public class GZIPCompression {
    ...
    public static byte[] compress(final String stringToCompress) {
        return compress(stringToCompress.getBytes(CHARSET));
    }
    ...
    public static byte[] decompressToBytes(final byte[] compressed) {
        if (Objects.isNull(compressed) || compressed.length == 0) {
            return null;
        }
