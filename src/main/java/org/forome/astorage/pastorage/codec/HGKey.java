...
public class HGKey {
	...
	public static Map<Chromosome, TabItem> hg19Tab = new LinkedHashMap<Chromosome, TabItem>() {{
	...
	public static Map<Chromosome, TabItem> hg38Tab = new LinkedHashMap<Chromosome, TabItem>() {{
	...
	public static Position decode(Assembly assembly, byte[] key) {
		Map<Chromosome, TabItem> tabs = getTab(assembly);

		long long_pos = Longs.fromBytes((byte)0, (byte)0, (byte)0, (byte)0, key[0], key[1], key[2], key[3]);

		Chromosome chromosome = null;
		TabItem tabItem = null;
		for (Map.Entry<Chromosome, TabItem> entry : tabs.entrySet()) {
			TabItem iTabItem = entry.getValue();
			if (iTabItem.start > long_pos) {
				break;
			}
		...
		return new Position(chromosome, (int) (long_pos - tabItem.start));
	}
